...
/**
 * Мобильный поиск: тонкая закреплённая строка сверху, по тапу — современный
 * полноэкранный оверлей с живыми подсказками (учитывает опечатки). Только на мобильных.
 */
...
function SearchOverlay({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-surface lg:hidden animate-fade-up">
      {/* строка ввода */}
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain p-4">
        {q.trim().length < 2 ? (
          <div>
            <div className="mb-2 text-xs font-bold uppercase tracking-wide text-ink-faint">Популярное</div>
            <div className="flex flex-wrap gap-2">
              {POPULAR.map((p) => (
                <button
                  key={p}
                  onClick={() => submit(p)}
                  className="rounded-full bg-surface-soft px-3.5 py-2 text-sm font-medium text-ink ring-1 ring-line active:scale-95"
                >
                  {p}
                </button>
              ))}
          ...
          <div className="flex items-center gap-2 px-1 py-6 text-ink-faint">
            <Loader2 className="h-4 w-4 animate-spin" /> Ищем…
          </div>
        ) : items.length === 0 ? (
          <div className="px-1 py-6 text-ink-muted">
