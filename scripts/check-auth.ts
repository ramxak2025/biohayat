...
async function main() {
  ...
  // Жёстко проваливаем только по-настоящему опасный случай: https без Secure.
  // Обратный случай (Secure на http) — следствие настройки NEXT_PUBLIC_SITE_URL:
  // на боевом https всё верно, но по http-адресу браузер такую cookie выбросит.
  const secure = /;\s*Secure/i.test(setCookie);
  if (isHttps) {
    check("cookie помечена Secure (сайт на https)", secure, "Secure не стоит");
  } else if (secure) {
    console.log("  ВНИМАНИЕ cookie помечена Secure, а сайт открыт по http —");
    console.log("           в браузере такая сессия не сохранится (проверьте NEXT_PUBLIC_SITE_URL)");
  } else {
    check("cookie без Secure (сайт на http)", true);
  }
  ...
  // Признак входа — форма выхода: она есть только у авторизованного и не зависит
  // от текстов на странице (они отличаются между версиями кабинета).
  check(
    "кабинет открывается после входа",
    html.includes("/account/logout"),
